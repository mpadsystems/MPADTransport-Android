...
import com.wneth.mpadtransport.databinding.ActivityDiscountBinding
...
import com.wneth.mpadtransport.viewmodels.configurations.DiscountActivityViewModel
import com.wneth.mpadtransport.views.fragments.SearchFragment

class DiscountActivity : AppCompatActivity(), SearchFragmentListener, RecyclerDialogEventListener {
    ...
    */
    ...
    private lateinit var viewModel: DiscountActivityViewModel
    private lateinit var viewBinding: ActivityDiscountBinding
    // =================================
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        * Code Below
        */
        viewModel = ViewModelProvider(this)[DiscountActivityViewModel::class.java]
        // =================================
        ...
        * Code Below
        */
        viewBinding = ActivityDiscountBinding.inflate(layoutInflater)
    ...
    private fun populateBuses() {
        viewModel.discounts.observe(this) {
            populateBuses(it)
        }
    }

    private fun populateBuses(discounts: List<DiscountModel>) {

        val menuRecyclerView: RecyclerView = findViewById(R.id.recyclerItems);
        val manager = GridLayoutManager(this, 1)
        menuRecyclerView.layoutManager = manager

        val data = discounts.map { discount ->
            mapOf(
                "id" to discount.id,
                "navigateTo" to "",
                "visible" to true,
                "title" to discount.description,
                "subTitle" to "${discount.fraction}%",
    ...
    override fun onSearchTextReceived(text: String) {
        viewModel.filterDiscounts(text)
    }
    ...
    override fun onRecyclerItemLongClicked(id: Int, position: Int, title: String, subTitle: String, navigateTo: String) {
        ...
        builder
            ...
            .setItems(arrayOf("Delete", "Disable", "Enable")) { dialog, which ->
                when (which) {
                    0 -> {
                        val discount = DiscountModel(id.toInt(), 0, "", "", 0, true,"")
                        viewModel.deleteBus(discount)
